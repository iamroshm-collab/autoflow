...
export async function POST(
  ...
  if (!["assigned", "accepted"].includes(allocation.status)) {
    return NextResponse.json({ error: "Job has already been started or completed." }, { status: 409 })
  }
  ...
  const updated = await prismaClient.technicianAllocation.update({
    ...
    data: {
      status: "in_progress",
      startedAt: new Date(),
      acceptedAt: allocation.acceptedAt ?? new Date(),
    },
  })

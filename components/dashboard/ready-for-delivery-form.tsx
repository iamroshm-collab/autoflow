...
interface ReadyForDeliveryFormProps {
  ...
  onJobCardUpdated?: (jobCardId: string) => void
  onRegistrationFilterChange?: (registrationNumber: string) => void
}
...
export function ReadyForDeliveryForm({
  ...
  useEffect(() => {
    console.debug("ReadyForDeliveryForm:prop selectedJobCardId", { selectedJobCardId })
  }, [selectedJobCardId])

  useEffect(() => {
    console.debug("ReadyForDeliveryForm:state jobCardId", { jobCardId: formData.jobCardId })
  }, [formData.jobCardId])
  ...
  useEffect(() => {
    if (searchInputFocused && allVehicles.length > 0 && (searchValue || "").trim().length === 0) {
      setShowRegistrationSuggestions(true)
      setRegistrationSuggestions(allVehicles)
      dropdownNav.resetHighlight()
    }
  }, [searchInputFocused, allVehicles, searchValue, dropdownNav])
  ...
  // Load all vehicles eligible for delivery search (Completed + Ready) on mount
  useEffect(() => {
    const loadReadyForDeliveryVehicles = async () => {
      try {
        const response = await fetch("/api/jobcards/delivered-suggestions?q=")
        const data = await response.json()
        setAllVehicles(Array.isArray(data) ? data : [])
      } catch (error) {
        console.error("Error loading ready-for-delivery vehicles:", error)
      }
    }
    loadReadyForDeliveryVehicles()
  }, [])
  ...
  useEffect(() => {
    ...
    const balance = Math.max(totalBill - advance - advancePayment, 0)
  ...
  }, [sparesTotal, serviceTotal, spareReturnsTotal, formData.discount, formData.advance, formData.advancePayment])

  useEffect(() => {
    if (formData.deliveryStatus === "Delivered" && !formData.deliveryDate) {
      setFormData((prev) => ({
        ...prev,
        deliveryDate: formatDateDDMMYY(new Date()),
      }))
    }
  }, [formData.deliveryStatus, formData.deliveryDate])
  ...
  const loadJobCard = async (jobCardId?: string, opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      setFormData({
        ...
        advance: Number(data.paidAmount || 0),
        ...
        nextServiceKm: data.nextServiceKm ? String(data.nextServiceKm) : "",
        nextServiceDate: data.nextServiceDate ? formatDateDDMMYY(new Date(data.nextServiceDate)) : "",
        deliveryDate: data.deliveryDate ? formatDateDDMMYY(new Date(data.deliveryDate)) : "",
      })
  ...
  const handleDeliveryStatusChange = (value: string) => {
    setFormData((prev) => {
      const updated = { ...prev, deliveryStatus: value }
      
      // When delivery status is set to "Delivered"
      if (value === "Delivered") {
        // Auto-set delivery date to today
        updated.deliveryDate = formatDateDDMMYY(new Date())
        
        // If maintenance type is Oil Change and ODO is not filled, urge user
        if (updated.maintenanceType === "Oil Change" && !updated.odo) {
          notify.warn("Please enter ODO reading for Oil Change service")
          setTimeout(() => {
            odoInputRef.current?.focus()
          }, 100)
        }
        
        // Auto-calculate next service values for Oil Change
        if (updated.maintenanceType === "Oil Change" && updated.odo) {
          const odoValue = Number(updated.odo)
          updated.nextServiceKm = String(odoValue + 10000)
          
          // Calculate next service date as one year from today
          const deliveryDate = new Date()
          deliveryDate.setFullYear(deliveryDate.getFullYear() + 1)
          updated.nextServiceDate = formatDateDDMMYY(deliveryDate)
        }
      }
  ...
  const handleAdvancePaymentChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => {
      const updated = { ...prev, advancePayment: value }
      
      // When user adds advance payment, auto-set payment status to Partial
      if (value > 0 && prev.jobcardPaymentStatus !== "Partial") {
        updated.jobcardPaymentStatus = "Partial"
      }
  ...
  const handleSave = async () => {
    ...
    // Validate delivery status requirements when marked as Delivered
    if (formData.deliveryStatus === "Delivered") {
      // Payment status must be either Partial or Completed
      if (formData.jobcardPaymentStatus !== "Partial" && formData.jobcardPaymentStatus !== "Completed") {
        errorAction("Payment status must be 'Partial' or 'Completed' when delivery is marked as Delivered")
        openPaymentStatusDropdown()
        return
      }
      ...
      if (!formData.deliveryDate || !formData.deliveryDate.trim()) {
        errorAction("Delivery date must be set when delivery is marked as Delivered")
        return
      }
      
      // If maintenance type is Oil Change, validate ODO and auto-calculated fields
      if (formData.maintenanceType === "Oil Change") {
        // ODO must be filled
        if (!formData.odo || !String(formData.odo).trim() || Number(formData.odo) === 0) {
          errorAction("ODO reading is required for Oil Change service")
          odoInputRef.current?.focus()
          return
        }
        
        // Next Service KM should be auto-calculated
        if (!formData.nextServiceKm) {
          const odoValue = Number(formData.odo)
          setFormData((prev) => ({
            ...prev,
            nextServiceKm: String(odoValue + 10000)
          }))
          notify.info("Next Service KM has been auto-calculated")
          return
        }
        
        // Next Service Date should be auto-calculated
        if (!formData.nextServiceDate) {
          const deliveryDate = new Date()
          deliveryDate.setFullYear(deliveryDate.getFullYear() + 1)
          setFormData((prev) => ({
            ...prev,
            nextServiceDate: formatDateDDMMYY(deliveryDate)
          }))
          notify.info("Next Service Date has been auto-calculated as one year from delivery date")
          return
        }
      }
    }
    ...
    try {
      ...
      const response = await fetch(`/api/jobcards/${formData.jobCardId}`, {
        ...
        body: JSON.stringify({
          ...
          serviceDate: formData.jobcardDate,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      onJobCardUpdated?.(deletedJobCardId)
          ...
          registrationSuggestions.map((suggestion, index) => (
              ...
              >
                ...
                <div className="text-sm text-gray-600">
                  {[suggestion.vehicleMake, suggestion.vehicleModel].filter(Boolean).join(" ") || "Vehicle"}
                </div>
              </button>
            ))
        ) : (
    ...
    <>
      ...
      <Card className="p-4 md:p-6">
        <div className="grid grid-cols-5 gap-4">
          ...
          {/* Row 2 */}
          <div className="grid gap-2">
            <Label htmlFor="jobCardNumber">Jobcard No</Label>
            <Input
              id="jobCardNumber"
          ...
          {/* Row 4 - Maintenance Type and Next Service Info */}
          <div className="grid gap-2">
          ...
          <div className="grid gap-2">
            ...
            <DatePickerInput
              ...
              onChange={(value) => setFormData((prev) => ({ ...prev, nextServiceDate: value }))}
              disabled={isLoading}
              placeholder="dd-mm-yy"
              format="dd-mm-yy"
            />
                  ...
                  <tr key={row.id} className="border-t [&>td]:align-middle">
                    ...
                    <td className="p-1">
                      <Select
                        ...
                        onValueChange={(value) =>
                          handleTechnicianChange(row.id, "workType", value)
                        }
                        disabled={isLoading}
                      >
                        <SelectTrigger className="h-10 text-sm" onFocus={() => handleTechnicianRowFocus(row.id)}>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>

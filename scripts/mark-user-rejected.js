...
if (!mobile) {
  console.error('Usage: node scripts/mark-user-rejected.js <10-digit-mobile>')
  process.exit(1)
}
...
async function main() {
  ...
  const backupPath = path.join(backupsDir, `${mobile}-appUser-backup-reject-${timestamp}.json`)
  ...
  console.log(JSON.stringify({ success: true, backup: backupPath, before: user, after: updated }, null, 2))
}
